...
pub fn part_one(map: Input) -> Option<u32> {
    ...
    }) = q.pop_front()
    {
        ...
        if opened.len() == flowing.len() || elapsed >= 30 {
            let released_at_end = wait_until_ending(30, elapsed, released, &opened, &map);
            max_released = max_released.max(released_at_end);
            continue;
        }
...
pub fn part_two(map: Input) -> Option<u32> {
    let dist_map = min_distances(&map); // key: (from, to), value: move_cost
    let flowing: HashSet<_> = map
        .iter()
        .filter(|(_, valve)| valve.flow_rate > 0)
...
mod tests {
    ...
    fn test_part_one() {
        let test_no = 1;
        let input = utils::get_test_input(2022, 16, test_no);
        let parsed_input = parse_input(&input);
        let expected = utils::get_test_result(2022, 16, 1, test_no)
            .parse()
            .unwrap();
    ...
    fn test_part_two() {
        ...
        let input = utils::get_test_input(2022, 16, test_no);
        let parsed_input = parse_input(&input);
        let expected = utils::get_test_result(2022, 16, 2, test_no)
            .parse()
            .unwrap();
